        ...
        >
            ...
            >
                ...
                <div className="flex-1 min-w-0">
                    ...
                    {!isExpanded && q.tags && q.tags.filter(t => !t.match(/^\d{8}$/) && !t.match(/^A\d{8}$/)).length > 0 && (
                        <div className="flex flex-wrap gap-1 mt-1.5">
                            {q.tags.filter(t => !t.match(/^\d{8}$/) && !t.match(/^A\d{8}$/)).slice(0, 4).map((tag, i) => (
                                <span key={i} className="inline-flex items-center gap-0.5 text-[10px] font-medium text-indigo-600 bg-indigo-50 border border-indigo-100 px-1.5 py-0.5 rounded-full">
                                    <span className="text-indigo-400">#</span>{tag}
                                </span>
                            ))}
                            {q.tags.filter(t => !t.match(/^\d{8}$/) && !t.match(/^A\d{8}$/)).length > 4 && (
                                <span className="text-[10px] text-indigo-400 font-medium">+{q.tags.filter(t => !t.match(/^\d{8}$/) && !t.match(/^A\d{8}$/)).length - 4}</span>
                            )}
                        </div>
                    )}
                </div>
            ...
            {isExpanded && (
                <div className="px-3 pb-3 animate-fade-in border-t border-gray-100 mt-0 max-w-full overflow-hidden">
                    ...
                    {/* Tags */}
                    {displayQ.tags && displayQ.tags.filter(t => !t.match(/^\d{8}$/) && !t.match(/^A\d{8}$/)).length > 0 && (
                        <div className="flex flex-wrap gap-1.5 mt-2 pt-2 border-t border-gray-100">
                            <span className="text-[10px] text-indigo-500 font-semibold mr-0.5 flex items-center gap-1">
                                <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" />
                                </svg>
                                Tags:
                            </span>
                            {displayQ.tags.filter(t => !t.match(/^\d{8}$/) && !t.match(/^A\d{8}$/)).map((tag, i) => (
                                <span key={i} className="inline-flex items-center gap-0.5 text-[10px] font-medium text-indigo-600 bg-indigo-50 border border-indigo-100 px-2 py-0.5 rounded-full hover:bg-indigo-100 transition-colors">
                                    <span className="text-indigo-300">#</span>{tag}
                                </span>
                            ))}
                        </div>
                    )}
