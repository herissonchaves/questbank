...
// Dynamic hierarchical tree with checkboxes, search, cascading selection, filtered counts, right-click rename (white theme)

const SubjectTree = ({ tree, filteredTree, activeSubjects, onSubjectsChange, onMoveNode, onRenameNode }) => {
    ...
    // Context menu state
    const [contextMenu, setContextMenu] = React.useState(null); // { x, y, path, level, label }
    ...
    const handleContextMenu = (e, fullPath, level, label) => {
        e.preventDefault();
        e.stopPropagation();
        setContextMenu({ x: e.clientX, y: e.clientY, path: fullPath, level, label });
    };

    const startRename = () => {
        if (!contextMenu) return;
        setEditingPath(contextMenu.path);
        setEditValue(contextMenu.label);
        setContextMenu(null);
    };
    ...
    const confirmRename = () => {
        ...
        if (newName && newName !== oldName && onRenameNode) {
            onRenameNode(editingPath, newName, parts.length - 1);
        }
        setEditingPath(null);
        setEditValue('');
    };
    ...
    // ─── Level labels for context menu ──────────────────────
    const getLevelLabel = (level) => {
        const labels = ['Disciplina', 'Tópico', 'Conteúdo', 'Assunto'];
            ...
            <div key={fullPath} className={indentClass}>
                <div 
                    className={`flex items-center gap-1.5 py-1 px-2 rounded-lg cursor-pointer group transition-all duration-150 ${
                        ...
                        isEditing ? ' ring-2 ring-brand-400 bg-brand-50' : ''
                    }`}
                    draggable={!isEditing}
                    onDragStart={(e) => {
                    ...
                    onContextMenu={(e) => handleContextMenu(e, fullPath, level, node.label)}
                >
                    ...
                    {isEditing ? (
                        <div className="flex items-center gap-1.5 flex-1 min-w-0">
                            ...
                            <input
                                ...
                                className="flex-1 min-w-0 text-sm bg-white border border-brand-300 rounded px-1.5 py-0.5 text-gray-800 focus:outline-none focus:ring-1 focus:ring-brand-500 focus:border-brand-500"
    ...
    return (
        <div className="flex flex-col h-full">
            ...
            {contextMenu && (
                <div
                    className="fixed z-[9999] bg-white rounded-lg shadow-2xl border border-gray-200 py-1 min-w-[180px] animate-fade-in"
                ...
                >
                    <div className="px-3 py-1.5 text-[10px] font-semibold text-gray-400 uppercase tracking-wider border-b border-gray-100">
                        {getLevelLabel(contextMenu.level)}
                    </div>
                    <button
                        onClick={startRename}
                        className="w-full text-left px-3 py-2 text-xs text-gray-700 hover:bg-brand-50 hover:text-brand-700 transition-colors flex items-center gap-2"
                    >
