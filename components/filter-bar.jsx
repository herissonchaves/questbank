...
const FilterBar = ({ filters, availableValues, onFilterChange, onClearFilters, resultCount, totalCount, ignoreUsed, onToggleIgnoreUsed, searchRef }) => {
    ...
    const hasActiveFilters = filters.search || filters.banca || filters.ano || filters.dificuldade || filters.tipo || filters.codigo || filters.orderById || ignoreUsed || filters.orderByRecent || filters.semResolucao;
    ...
    return (
        <div className="flex flex-col gap-2 p-3 border-b border-gray-200">
            ...
            <div className="flex flex-wrap gap-2 items-center">
                ...
                {/* Sem Resolução Toggle */}
                <button
                    onClick={() => onFilterChange('semResolucao', !filters.semResolucao)}
                    className={`text-xs font-medium px-2.5 py-1.5 rounded-lg border transition-all flex items-center gap-1 ${filters.semResolucao
                            ? 'bg-amber-50 border-amber-200 text-amber-700 shadow-sm'
                            : 'bg-gray-50 border-gray-200 text-gray-500 hover:text-gray-700 hover:border-gray-300'
                        }`}
                    title="Mostrar apenas questões que não têm resolução"
                >
