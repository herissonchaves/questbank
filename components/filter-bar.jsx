const TagFilter = ({ selectedTags, availableTags, onChange }) => {
    ...
    // Filter available tags based on search input and already selected tags
    ...
    return (
        <div className="relative flex-1 min-w-[200px]" ref={containerRef}>
            <div 
                className={`flex flex-wrap gap-1.5 p-1.5 bg-gray-50 border rounded-lg transition-all min-h-[38px] cursor-text ${
                    isOpen ? 'border-indigo-400 ring-2 ring-indigo-500/20' : 'border-gray-200 hover:border-gray-300'
                }`}
            ...
            >
                {selectedTags.map((tag, i) => (
                    <span 
                        key={i} 
                        className="inline-flex items-center gap-1 text-[11px] font-semibold text-indigo-700 bg-indigo-50 border border-indigo-100 px-2 py-0.5 rounded-full animate-fade-in"
                    >
                        #{tag}
                        <button
                            type="button"
                ...
                <input
                    ...
                    placeholder={selectedTags.length === 0 ? "Filtrar por tags..." : ""}
                    className="flex-1 min-w-[80px] bg-transparent text-xs text-gray-700 placeholder-gray-400 focus:outline-none"
                />
            </div>

            {/* Suggestions Dropdown */}
            {isOpen && suggestions.length > 0 && (
                <div className="absolute left-0 right-0 top-full mt-1 bg-white rounded-xl shadow-2xl border border-gray-100 z-[100] py-1.5 overflow-hidden animate-slide-up">
                    ...
                    {suggestions.map((tag, index) => (
                        <button
                            ...
                            className={`w-full text-left px-3 py-2 text-xs transition-all flex items-center justify-between ${
                                index === highlightedIndex ? 'bg-indigo-50 text-indigo-700' : 'text-gray-600 hover:bg-gray-50'
                            }`}
                        >
                            <span>#{tag}</span>
                ...
                <div className="flex flex-wrap gap-2 items-center pt-1 animate-fade-in">
                    ...
                    <div className="flex items-center gap-1.5 flex-1">
                        ...
                        <TagFilter 
                            ...
                            availableTags={availableValues.tags || []}
                            onChange={(tags) => onFilterChange('tag', tags)}
                        />
                    </div>
                </div>
            )}
